...
@section('content')
    ...
    <div class="card event-view-card">
        ...
        <div class="event-view-meta">
            ...
            @if (in_array($event->attendance_type, ['virtual', 'both']))
                <div class="event-view-item">
                    <span class="event-view-label">Meet Link</span>
                    <span class="event-view-value">
                        @if ($event->meet_link)
                            <a href="{{ $event->meet_link }}" target="_blank" rel="noopener noreferrer" style="color: #1B6CA8; text-decoration: underline;">{{ $event->meet_link }}</a>
                        @else
                            Not set
                        @endif
                    </span>
        ...
        <div class="event-assets-grid">
            ...
            @if ($event->program_file_path)
                <div class="event-asset-card" data-program-path="{{ $event->program_file_path }}" data-program-name="{{ $event->program_file_name ?? basename($event->program_file_path) }}">
                    ...
                    <div class="template-preview" style="margin-bottom: 1rem; background: #f5f5f5; border-radius: 8px; overflow: hidden;">
                        <?php
